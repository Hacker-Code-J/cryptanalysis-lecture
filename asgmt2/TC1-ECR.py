...
def make_one_tmto_table(P0, m, t, ell):
    ...
    for _ in range(0,m):
        ...
        SP = [0] + [random.randint(0, 255) for _ in range(3)]
        EP, list_key = chain_EP(SP, P0, t)

        for i in range(0,len(list_key)):
            list_table.append(list_key[i])
        
        # { (Key=EP, Value=SP) }
        SP_int = list2int(SP)
        EP_int = list2int(EP)
        tmto_dic[EP_int] = SP_int # EP is Key
    ...
    for i in range(0,len(list_table)):
        list_int = list2int(list_table[i])
        list_dic[list_int] = i
    H = len(list_dic)
    return H / (m * t)
...
def make_all_tmto_tables(P0, m, t, num_of_tables):
    ...
    for ell in range(num_of_tables):
        ...
        sys.stdout.flush()
        ecr_table.append(make_one_tmto_table(P0, m, t, ell))
...
for idx, ecr in enumerate(ecr_table):
    print(f"ECR-{idx} = {ecr:.4f}")
