"""
=========================================
DTC1 - Double Encryption : DTC1_Enc(), DTC1_Dec()
 - 블록크기, 키크기: 32비
 - DT1_Dec(), DTC1_Enc()의 암호키
    key = [k1, k2]

    CT
    = DTC1_Enc(PT, K)
    = TC1_Enc( TC1_Enc(PT, k1), k2)

 - version 1: 
      동일한 라운드함수만 사용함 (마지막 라운드도 동일)
      키 스케줄 없음
      복호화 추가
=========================================
"""

NUM_ROUND = 10 # Round Number of TC1
# Round Number of DTC1 = 20
...
#-- Additional Double Encryption
def DTC1_Enc(PT, K): # K = [K[0], K[1]] = [key1, key2]
    Mid = TC1_Enc(PT, K[0])
    CT = TC1_Enc(Mid, K[1])
    return CT
...
def main():
    ...
    key1 = [0, 1, 2, 3]
    key2 = [4, 5, 6, 7]
    K = [key1, key2] # Key for Double Encryption
